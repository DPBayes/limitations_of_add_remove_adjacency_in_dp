...
from auditing_utils import get_privacy_profile, convert_logit_to_prob
import warnings
import pickle
...
class Learner:
    ...
    def run(self):
        ...
        ys = [i for i in range(0,num_classes) if i != y.item()]
        ...
        for r in range(self.args.start_run_index, self.args.stop_run_index):
            ...
            for curr_step in range(self.args.steps):
                ...
                if (batch_flat == target_sample_flat).all(dim=1).any() and b == 1: # if the target sample is present in the batch
                    ...
                    data[target_index_in_batch] = x ## assign the input canary to the target index
                ...
                with torch.no_grad():
                    model.eval()
                    model_logits = model(torch.unsqueeze(x,dim=0))
                    attack_scores[r - self.args.start_run_index,curr_step] = model_logits[0][y.item()].item() - model_logits[0][y_prime.item()].item()
        ...
        with open(os.path.join(self.directory, 'input_space_y_results_q_{}_r_{}_s_{}_lr_{}_runs_{}_{}.pkl'.format(
                sample_rate,
