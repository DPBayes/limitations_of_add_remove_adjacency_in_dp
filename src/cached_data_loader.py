...
class CachedFeatureLoader:
    ...
    def _subsample_shots(self, features: torch.Tensor, labels: torch.Tensor, classes: list, shots: int,task:str="train"):
        ...
        if shots != -1:
            ...
            # print(mia_training_indices)
            self.mia_training_indices = mia_training_indices ## to be retrieved
            ...
            if task == "train":
                return all_selected_features[:len(mia_training_indices),:], all_selected_labels[:len(mia_training_indices)], class_mapping
            elif task == "tune":
                return all_selected_features[len(mia_training_indices):,:], all_selected_labels[len(mia_training_indices):], class_mapping
            else:
