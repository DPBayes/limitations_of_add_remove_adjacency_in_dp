...
from auditing_utils import get_privacy_profile, calculate_statistic, convert_logit_to_prob
import warnings
import pickle
...
class Learner:
    ...
    def run(self):
        ...
        print(f"Chosen Target Sample: {target_sample_index}")
        ...
        ## get the reference gradient from (x,y)
        grad_ref = torch.autograd.grad(self.loss(model(x), y), model.parameters(), retain_graph=False, create_graph=False)
        grad_ref_vector = torch.cat([g.view(-1) for g in grad_ref]) ## reference gradient vector
        optimizer = torch.optim.SGD([x_prime], lr=self.args.learning_rate)
        ...
        print("-------------------------------------- Auditing with Crafted Input --------------------------------------")
        ...
        with open(os.path.join(self.directory, 'input_space_x_results_q_{}_r_{}_s_{}_lr_{}_runs_{}_{}.pkl'.format(
            sample_rate,
